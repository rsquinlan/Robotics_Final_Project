...
while robot.step(timestep) != -1:
    ...
    if mode == 'camera':
        ...
        if largest_contour_center['m00']==0:
            ...
            cx = (largest_contour_center['m10']/largest_contour_center['m00'])
            cy = (largest_contour_center['m01']/largest_contour_center['m00'])

        img = cv2.circle(img, (cx,cy), radius=0, color=(0, 0, 255), thickness=-1)
        cv2.imshow("Image", img)
        cv2.waitKey(0)
   ...
   ##################
    ...
    if mode == "autonomous":
        ...
        ratio = abs(vL/vR)
        if (ratio > 1):
            vR = math.copysign(MAX_SPEED/ratio, vR)
            vL = math.copysign(MAX_SPEED, vL)
        else:
